...
if ($entradaOK) {//Si los campos son correctos
    try { // Bloque de código que puede tener excepciones en el objeto PDO
        ...
        $sqlVaciarTabla = "TRUNCATE TABLE T02_Departamento";
        ...
        $sql = <<<EOD
                    ...
                    (:CodDepartamento, :DescDepartamento, :FechaCreacion, :VolumenNegocio, :FechaBaja);
EOD;
        ...
        for ($numeroDepartamento = 0; $numeroDepartamento<$numeroDepartamentos; $numeroDepartamento++){//Recorremos los departamentos
            ...
            $timestampFechaCreacion=$archivoXML->getElementsByTagName("FechaCreacion")->item($numeroDepartamento)->nodeValue;//Guardamos el valor del elemento de la fecha de creación del departamento
            settype($timestampFechaCreacion, 'integer');
            
            $timestampFechaBaja=$archivoXML->getElementsByTagName("FechaBaja")->item($numeroDepartamento)->nodeValue;//Guardamos el valor del elemento de la fecha de baja
            if(empty($timestampFechaBaja)){//Si el elemento de la feha de baja está vacío
                ...
                settype($timestampFechaBaja, 'integer');
            }
            ...
            $parametros = [":CodDepartamento" => $CodDepartamento,
                           ...
                           ":FechaCreacion" => $timestampFechaCreacion,
                           ":VolumenNegocio" => $VolumenNegocio,
                           ":FechaBaja" => $timestampFechaBaja];
